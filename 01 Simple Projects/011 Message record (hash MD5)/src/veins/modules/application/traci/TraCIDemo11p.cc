...
//used in MD5sum function (Crypto++)
...
std::string TraCIDemo11p::hashMD5(TraCIDemo11pMessage* wsm) {
    // Create md5sum from: senderAddress + ";" + demoData
    using namespace CryptoPP;

    std::string msg = std::to_string(wsm->getSenderAddress()) + ";" + wsm->getDemoData();
    std::string digest;
    ...
    std::string md5Hash;
    HexEncoder encoder(new StringSink(md5Hash));
    encoder.Put((const byte*)&digest[0], digest.size());
    encoder.MessageEnd();

    std::cout << "Message: " << msg << endl;
    std::cout << "MD5sum:  " << md5Hash << endl;
    std::cout << "------------------------------" << endl;
...
void TraCIDemo11p::storeMsg(TraCIDemo11pMessage* wsm, std::string hash) {
    //get message fields
    int serialWsm = wsm->getSerial();
    LAddress::L2Type srcWsm = wsm->getSenderAddress();
    std::string dataWsm = wsm->getDemoData();
