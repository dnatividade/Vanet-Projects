...
void TraCIDemoRSU11p::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        //std::cout << "RSU ID: " << myId << endl; //print RSU ID
        TraCIDemo11pMessage* wsm = new TraCIDemo11pMessage(); //create a message called wsm

        //send message to node[33] - enlace layer ID 214
        populateWSM(wsm, 214, 0); //fill the principal message fields [wsm = message] [34 = send a message to vehicle ID=34] [0 = any serial number]
        scheduleAt(simTime() + 2, wsm); //Make an appointment for 65 seconds from now [using self messages]
    }
}
...
{
    TraCIDemo11pMessage* ack = check_and_cast<TraCIDemo11pMessage*>(frame);

    //if received message = "msg received"...
    if (strcmp(ack->getDemoData(), "msg received") == 0) {
        msgReceived = 1; //set Flag to cancel sending
        std::cout << "RSU | ACK received! Stop to sending message!" << " | SimTime: " << simTime() << endl; //print information
    }
}
